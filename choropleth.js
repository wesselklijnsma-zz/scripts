...
function getMax(arr, prop) {
    ...
    for (var i=0 ; i<keys.length ; i++) {
        if (!max || parseInt(arr[keys[i]][prop]) > parseInt(max))
            max = arr[keys[i]][prop];
    }
    return max;
}
...
info.update = function (props) {
    this._div.innerHTML = (props ?
        '<b>' + props.gemeentena + '</b><br />' + colors[props.gemeentena].total + ' zinnen in akkoord'
        : 'Beweeg je muis <br> over een gemeente');
};
...
var base = {
    ...
    "S\u00fadwest-Frysl\u00e2n": {"colors": "#ffffff", "total": 0.0},
